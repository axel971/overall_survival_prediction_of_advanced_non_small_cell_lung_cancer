...
from model.model import CNN_3D_with_clinics, CNN_3D_testing_with_clinics
...
def main():
    ...
    clinics = np.array(data[['Age', 'Sex', 'PackYrs','BMI', 'CCI', 'CHD', 'CAD', 'CHF', 'AtrialFibFlutter', 'COPD', 'Diabetes', 'HTN', 'HLD']], dtype = np.float32)     
    ...
    for iFold in range(nFold):
        ...
        model = CNN_3D_with_clinics(image_size+[1], (clinics.shape[1]), (1,))
        optimizer = optimizers.Adam(learning_rate=1e-4)
        training_generator = Generator3D_with_clinics(X[train_id], clinics[train_id], times[train_id], events[train_id], batch_size = 10)
        ...
        model_testing = CNN_3D_testing_with_clinics(image_size + [1], (clinics.shape[1]))
        model_testing.set_weights(model.get_weights())
        
        
        
        train_pred_log_risk = model_testing.predict([X[train_id], clinics[train_id]], verbose = 0)
        ...
        testing_pred_log_risk = model_testing.predict([X[test_id], clinics[test_id]], verbose = 0)
