...
def main():
    ...
    file_dir = str(ROOT_DIR / '../data/raw_data/clinics/training_clinics.xls')
    ...
    file_volume_dir = str(ROOT_DIR / '../data/raw_data/tumor_volumes/training_volumes.xls')
    ...
    for iSimulation in range(nSimulation):
        ...
        model.fit(X = scaler.transform(clinics.loc[train_id, :]), T = times[train_id], E = events[train_id], max_features = 'sqrt', max_depth = 10, min_node_size = 10)
