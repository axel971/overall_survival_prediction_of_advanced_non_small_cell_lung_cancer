...
def main():
    ...
    radiomic_feature = radiomic_features.dropna(axis = 1)
    ...
    for iSimulation in range(nSimulation):    
        ...
        model.fit(X = scaler.transform(radiomic_features.loc[train_id, :]), T = times[train_id], E = events[train_id], init_method='zeros', l2_reg = 1e-3, lr = 0.00001, tol = 1e-4, max_iter = 100, verbose = 0)
