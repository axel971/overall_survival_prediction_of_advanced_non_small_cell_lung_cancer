...
def main():
    ...
    for iSimulation in range(nSimulation):    
        ...
        model = CoxnetSurvivalAnalysis(l1_ratio = 1., n_alphas = 1000000, alpha_min_ratio = 'auto', max_iter=1000000)
        model.fit(scaler.transform(clinics.loc[train_id, :]), y[train_id])
        ...
        train_pred_log_risk = model.predict(scaler.transform(clinics.loc[train_id,:]))         
        ...
        testing_pred_log_risk = model.predict(scaler.transform(clinics.loc[test_id, :])) 
