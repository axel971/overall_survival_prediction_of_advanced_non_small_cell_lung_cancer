...
def main():
    ...
    for iSimulation in range(nSimulation):
        ...
        training_data = np.concatenate((scaler_clinics.transform(clinics.loc[train_id, :]), pca_training.transform(scaler_radiomicFeatures.transform(radiomic_features.loc[train_id, :]))), axis =1)

        # Retrain model with best alpha on the whole training cohort 
        model = CoxnetSurvivalAnalysis(l1_ratio = 0.9, n_alphas = 1000000, alpha_min_ratio = "auto") # alphas = [gcv.best_params_["coxnetsurvivalanalysis__alphas"][0]])
        model.fit(training_data, y[train_id])
        ...
        train_pred_log_risk = model.predict(training_data)         
        ...
        testing_data = np.concatenate((scaler_clinics.transform(clinics.loc[test_id, :]), pca_training.transform(scaler_radiomicFeatures.transform(radiomic_features.loc[test_id, :]))), axis =1)
        testing_pred_log_risk = model.predict(testing_data) 
