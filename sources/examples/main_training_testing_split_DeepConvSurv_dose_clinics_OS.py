...
from model.generator_array import Generator3D_imaging_clinics
from model.model_DeepConvSurv import DeepConvSurv_imaging_clinics, DeepConvSurv_imaging_clinics_testing
...
def main():
    ...
    risk_score_dir = str(ROOT_DIR / '../data/output/OS/DeepSurv_DeepConvSurv/risk_scores')
    ...
    for n in range(n_subject):
        X[n], affines[n] =  get_nii_data_and_affine_matrix(dose_dir + '/' + subject_names[n] + '_resampled_boundingBox_doses.nii.gz' )
    ...
    for iSimulation in range(nSimulation):
        ...
        training_data = X[train_id]
        training_clinics = np.array(scaler.transform(clinics.loc[train_id,:]), dtype = np.float32)
        training_generator = Generator3D_imaging_clinics(training_data, training_clinics, times[train_id], events[train_id], batch_size = 10)
        ...
        model = DeepConvSurv_imaging_clinics(image_size+[1], (training_clinics.shape[1]), (1))
        ...
        model_testing = DeepConvSurv_imaging_clinics_testing(image_size + [1], (training_clinics.shape[1]))
        model_testing.set_weights(model.get_weights())
        
        train_pred_log_risk = model_testing.predict([training_data, training_clinics], verbose = 0)
        ...
        testing_data = X[test_id]
        testing_clinics = np.array(scaler.transform(clinics.loc[test_id, :]), dtype = np.float32)
        testing_pred_log_risk = model_testing.predict([testing_data, testing_clinics], verbose = 0)
