...
from model.model_DeepConvSurv import DeepConvSurv_imaging_dose_clinics, DeepConvSurv_imaging_dose_clinics_testing
...
def main():
    ...
    for iSimulation in range(nSimulation):
        ...
        model = DeepConvSurv_imaging_dose_clinics(image_size+[1], image_size+[1], (training_clinics.shape[1]), (1))
        ...
        model_testing = DeepConvSurv_imaging_dose_clinics_testing(image_size + [1], image_size+[1], (training_clinics.shape[1]))
        model_testing.set_weights(model.get_weights())
                
        train_pred_log_risk = model_testing.predict([training_data, doses[train_id], training_clinics], verbose = 0)
        ...
        testing_pred_log_risk = model_testing.predict([testing_data, doses[test_id], testing_clinics], verbose = 0)
