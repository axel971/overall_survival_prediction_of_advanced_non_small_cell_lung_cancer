...
def main():
    ...
    for iSimulation in range(nSimulation):
        ...
        #train_id, test_id = train_test_split(indices, test_size = 0.3, random_state = iSimulation, stratify = pandas.concat((pandas.DataFrame(events), pandas.DataFrame(institutions)), axis = 1))   
        

        print("training dataset size: " + str(len(train_id)))
        print("testing dataset size: " + str(len(test_id)))
