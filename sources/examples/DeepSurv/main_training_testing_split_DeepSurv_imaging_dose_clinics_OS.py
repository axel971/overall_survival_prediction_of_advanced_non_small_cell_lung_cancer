...
def main():
    ...
    for iSimulation in range(nSimulation):
        ...
        training_data = np.array(np.concatenate((scaler_clinics.transform(clinics.loc[train_id, :]), pca_img_training.transform(scaler_img_radiomicFeatures.transform(radiomic_img_features.loc[train_id, :])), pca_dose_training.transform(scaler_dose_radiomicFeatures.transform(radiomic_dose_features.loc[train_id, :])) ), axis =1), dtype = np.float32)
        ...
        ##### Prediction ####
        testing_data = np.array(np.concatenate((scaler_clinics.transform(clinics.loc[test_id, :]), pca_img_training.transform(scaler_img_radiomicFeatures.transform(radiomic_img_features.loc[test_id, :])), pca_dose_training.transform(scaler_dose_radiomicFeatures.transform(radiomic_dose_features.loc[test_id, :])) ), axis =1), dtype = np.float32)
